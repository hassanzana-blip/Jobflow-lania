...
const worker = new Worker(
  ...
  async (task) => {
    ...
    if (task.name === "delete_account") {
      const [request] =
        await sql`select id,state from deletion_requests where user_id=${userId}`;
      if (!request || request.state === "completed") return;
      await deleteCandidateData(userId, {
        async stopWork(id) {
          const [sub] =
            await sql`select stripe_subscription_id from subscriptions where user_id=${id}`;
          ...
          await sql`update profiles set state='deleting' where user_id=${id}`;
          await sql`update agent_runs set status='canceled' where user_id=${id} and status in ('queued','running')`;
          await sql`update notifications set status='canceled' where user_id=${id} and status='pending'`;
        },
        async listObjects(id) {
          return (
            await sql`select bucket,object_key from documents where user_id=${id}`
        ...
        async complete(id) {
          await sql`update deletion_requests set state='completed',completed_at=now(),error_code=null where user_id=${id}`;
        },
      });
      return;
    }
    const [profile] =
      await sql`select state from profiles where user_id=${userId}`;
    ...
    if (task.name === "daily_search") {
      ...
      const [p] =
        await sql`select roles,occupation_ids,locations from candidate_preferences where user_id=${userId}`;
      await Promise.all(
        sources.map((s) =>
          s.search({
            query: p.roles.join(" "),
...
worker.on("failed", async (task) => {
  if (task?.name === "delete_account")
    await sql`update deletion_requests set state='retry_required',error_code='DELETE_FAILED' where user_id=${task.data.userId}`;
});

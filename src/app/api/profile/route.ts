  ...
  try {
    ...
    const version = await sql.begin(async (tx) => {
      ...
      for (const f of p.facts)
        await tx`insert into jobbflow.candidate_facts(id,user_id,profile_version,kind,fact,source_quote,confirmed) values(${randomUUID()},${user.id},${row.version},${f.kind},${f.text},${f.text},true)`;
      return row.version;
    });

...
export async function POST(req: Request) {
  try {
    ...
    const sql = database();
    const [a] =
      await sql`insert into applications(id,user_id,job_id,status) select ${randomUUID()},${user.id},id,'preparing' from jobs where id=${jobId} and removed_at is null and (deadline is null or deadline>now()) on conflict(user_id,job_id) do update set user_id=excluded.user_id returning *`;
...
export async function PATCH(req: Request) {
  try {
    ...
    const a = await sql.begin(async (tx) => {
      ...
      await tx`insert into application_events(user_id,application_id,event_type) values(${user.id},${p.id},${`candidate_status:${p.status}`})`;
      return row;
    });
